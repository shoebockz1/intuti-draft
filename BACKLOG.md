...
- **In progress** — Auto-incrementing build version display. A small badge showing `v{commit count} ({short SHA})`, derived automatically at build time (`git rev-list --count HEAD` + `git rev-parse --short HEAD`) — no manual version-bumping step to forget, ticks up on its own with every commit. Shown on the page and in `GET /api/health`, so after any deploy it's a glance to confirm the live site actually picked up the latest commit, instead of the bundle-hash-comparison trick used so far.
